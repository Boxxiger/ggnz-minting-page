    ...
    KIP17Pausable
{
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, idstr, baseExtension))
                : "";
    }

    function mintBatch(address to, uint256 _mintAmount) external payable {
        uint256 supply = totalSupply();
        require(_mintAmount > 0);
        require(_mintAmount <= maximum);
    ...
    function transferBatch(address[] calldata tos, uint256[] calldata ids)
        external
    {
    ...
    function doNotUseThis() public onlyOwner {
